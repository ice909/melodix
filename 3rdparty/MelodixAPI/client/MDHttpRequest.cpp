...
MDHttpRequestInput::MDHttpRequestInput() {
    initialize();
}

MDHttpRequestInput::MDHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void MDHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void MDHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void MDHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    MDHttpFileElement file;
...
MDHttpRequestWorker::MDHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
MDHttpRequestWorker::~MDHttpRequestWorker() {
...
QMap<QString, QString> MDHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

MDHttpFileElement MDHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return MDHttpFileElement();
}

QByteArray *MDHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void MDHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void MDHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void MDHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void MDHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  MDHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString MDHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void MDHttpRequestWorker::execute(MDHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<MDHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, MDCompressionType::Gzip));
        } else {
    ...
    if (MDHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*MDHttpRequestWorker::sslDefaultConfiguration);
    }
...
void MDHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void MDHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void MDHttpRequestWorker::process_response(QNetworkReply *reply) {
...
QByteArray MDHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray MDHttpRequestWorker::compress(const QByteArray& input, int level, MDCompressionType compressType) {
...
QSslConfiguration *MDHttpRequestWorker::sslDefaultConfiguration;
